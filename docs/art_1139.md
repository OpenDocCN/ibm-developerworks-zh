...
当我们使用 user_with_ddl_permission 用户执行 create user mapping 时，有可能遇到授权的错误消息，比如：
...
除了可以应用 set session authorization 设置 session user 外，我门还可以通过 set session_user=user 完成相同的功能。
