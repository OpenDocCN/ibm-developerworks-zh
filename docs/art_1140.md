...
从输出结果可以看到总共有 4 个 chunk，分片 rep_shard1 有 2 个 chunk，分片 rep_shard2 有 2 个 chunk，分片后按照分片值 hash 后，存放到对应不同的分片。现在我们来将使用同一查询，看看在基于 Hashed 分片和基于 Ranged 分片的效果，以下为命令和部分输出示例：
