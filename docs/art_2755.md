...
> 原文：[`developer.ibm.com/zh/articles/j-lo-hotswapcls/`](https://developer.ibm.com/zh/articles/j-lo-hotswapcls/)
...
![Java 类加载器层次结构图](img/86ba6d8f63a9376bb75a5750c6baef10.png)
...
这四种类加载器的层次关系图如 图 1\. Java 类加载器层次结构图 所示。一般来说，这四种类加载器会形成一种父子关系，高层为低层的父加载器。在进行类加载时，首先会自底向上挨个检查是否已经加载了指定类，如果已经加载则直接返回该类的引用。如果到最高层也没有加载过指定类，那么会自顶向下挨个尝试加载，直到用户自定义类加载器，如果还不能成功，就会抛出异常。Java 类的加载过程如 图 2\. Java 类的加载过程 所示。

##### 图 2\. Java 类的加载过程

![图 2\. Java 类的加载过程](img/5548b8b7f27f3a1f98ba73157c180d69.png)
...
![图 3\. 热替换前的运行结果](img/bef45ed4455d2205ee299028f6b7b57b.png)
...
![图 4\. 热替换后的运行结果](img/7f31d633b0e01aad6d929ac952514746.png)
