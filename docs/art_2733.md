...
> 原文：[`developer.ibm.com/zh/articles/j-lo-spring-principle/`](https://developer.ibm.com/zh/articles/j-lo-spring-principle/)
...
![图 1 .Spring 框架的总体架构图](img/3fdf6d897ec61b10dbe6c83c3bbd61d6.png)
...
![图 2\. 三个组件关系](img/60f9cf59b72ab6fe8c0a609442284ee4.png)
...
![图 3\. Bean 工厂的继承关系](img/a7d539def909832e017a12c8fdc6ee94.png)
...
![图 4\. Bean 定义的类层次关系图](img/a01c22dc04306a315980299850867979.png)

Bean 的定义就是完整的描述了在 Spring 的配置文件中你定义的 <bean>节点中所有的信息，包括各种子节点。当 Spring 成功解析你定义的一个 <bean>节点后，在 Spring 的内部就被转化成 BeanDefinition 对象。以后所有的操作都是对这个对象完成的。</bean></bean>

Bean 的解析过程非常复杂，功能被分的很细，因为这里需要被扩展的地方很多，必须保证有足够的灵活性，以应对可能的变化。Bean 的解析主要就是对 Spring 配置文件的解析。这个解析过程主要通过下图中的类完成：

##### 图 5\. Bean 的解析类

![图 5\. Bean 的解析类](img/a37cee5a37c134de931a6222b32b463f.png)
...
![图 6\. Context 相关的类结构图](img/d433b9d578b3bbef5a248d8b6872b037.png)
...
![图 7\. Resource 相关的类结构图](img/55d23158923de60dfbe6d2a304548b8c.png)
...
![图 8\. Context 和 Resource 的类关系图](img/9fe07193c92122cff02f442d7946eb4d.png)
...
![图 9\. DefaultListableBeanFactory 类继承关系图](img/db99c744d66d39859039a49bee923bc9.png)
...
![图 10\. 创建 BeanFactory 时序图](img/1e617c47bc6438a75631b54a3107ed55.png)

Bean 的解析和登记流程时序图如下：

##### 图 11\. 解析和登记 Bean 对象时序图

![图 11\. 解析和登记 Bean 对象时序图](img/91efea0b9228a32a255d7e72b0f246a4.png)
...
![图 12.Bean 实例创建流程图](img/0bc00bfc14614507b5e73754fb84de6b.png)

如果是普通的 Bean 就直接创建他的实例，是通过调用 getBean 方法。下面是创建 Bean 实例的时序图：

##### 图 13.Bean 实例创建时序图

![图 13.Bean 实例创建时序图](img/e47cd0758228434ef4fa917f0dbfc183.png)

还有一个非常重要的部分就是建立 Bean 对象实例之间的关系，这也是 Spring 框架的核心竞争力，何时、如何建立他们之间的关系请看下面的时序图：

##### 图 14.Bean 对象关系建立

![图 14.Bean 对象关系建立](img/f95c46c83965b45bf2ecd4d5e4eb2578.png)
...
![图 15\. Proxy 类结构](img/a6ecf5fd23f6ca2bb9541d6e2ce999be.png)
...
![图 16\. 创建代理对象时序图](img/a524f6e8b63d9911bdb6c98e2f85c9c5.png)
...
![图 17\. Jdk 动态代理的类图](img/c3ae840efb35cfcd997162d86a259275.png)
...
![图 18.Spring 代理对象的产生](img/1b6ca1d72d5750944cb89ac004ed614b.png)
...
![图 19.Spring 调用拦截器](img/82a71c92aa033ec2c211f03fcb57cc48.png)
...
![图 20\. 代理模式的结构](img/e454e9bb561fea2d21fdce7495075553.png)
...
![图 21\. Spring 中使用代理模式的结构图](img/3b9448a8c3540d65d2dcbe2b5b472f9f.png)
...
![图 22\. 策略模式的结构](img/01a670a343b4c4b5d4e64570696262cc.png)
...
![图 23\. Spring 中策略模式结构图](img/c83ed6bfd4b3842b712779150afb155f.png)
