...
可以看到，在执行了 Create Model 方法后，生成了一个新的 model_id，这个就是新创建的定制翻译模型的 ID，后续都会使用到这个 model_id。另外，此时的状态是 dispatching，表示这个定制翻译模型还没有训练完成。后续可以使用 List Models 方法查看这个定制翻译模型的状态。
