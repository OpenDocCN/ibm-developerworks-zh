...
`totalSelectedValues` 的第一个参数是集合对象，而第二个参数是 `Predicate` 函数接口。 因为参数类型是函数接口 (`Predicate`)，所以我们现在可以将一个 lambda 表达式作为第二个参数传递给 `totalSelectedValues` 。例如，如果我们想仅对一个 `numbers` 列表中的 *偶数值* 求和，可以调用 `totalSelectedValues` ，如下所示：
