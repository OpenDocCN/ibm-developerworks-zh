...
上述代码中显示了最新的 iPhone 模拟器之一以及两种 Android。Android 1.6 不支持我们上述的这些检测。但实际上只要使用 Google Gear 它就能支持除视频之外的所有这些特性。它们就相当于是 API（就功能而言），但它们并不符合 Web 标准，因此会得到清单 4 中所显示的结果。将它与 Android 2.1 做个对照，后者支持所有这些特性。
