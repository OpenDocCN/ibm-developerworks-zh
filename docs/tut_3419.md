# Archived | Hadoop 教程：Hello World 与 Java、Pig、Hive、Flume、Fuse、Oozie，以及 Sqoop 与 Informix、DB2 和 MySQL

> 原文：[`developer.ibm.com/zh/tutorials/dm-1209hadoopbigdata/`](https://developer.ibm.com/zh/tutorials/dm-1209hadoopbigdata/)
...
其中一个引起困惑的地方是，”我在哪里可以获得处理大数据的软件？” 本文中的示例都以 Hadoop 的免费 Cloudera 发行版为基础，其名称为 CDH（全称为 Cloudera distribution including Hadoop，包括 Hadoop 的 Cloudera 发行）。Cloudera 网站将其作为一个 VMWare 映像提供。IBM 最近宣布，它正在将其大数据平台移植到在 CDH 上运行。
...
![Hadoop 架构](img/15aed429111c9d56c43df2f8872626ca.png)
...
![一个 64 位虚拟来宾的 BIOS 设置](img/fc712490124b4526c933762fdd459f20.png)
...
下载一个映像。CDH4 映像是此处可提供的最新产品： CDH4 映像 。
...
![虚拟机映像屏幕截图](img/b5b3740cabbe2c3e708d3a3a30f14c15.png)
...
![对虚拟机增加 RAM](img/6a1e078022b367ac0421dae7144b9955.png)

如图 5 所示，您可以将网络设置更改为 **桥接** 。使用此设置，虚拟机将会获得自己的 IP 地址。如果这会在您的网络上引起问题，那么您可以选择使用网络地址转换 (NAT)。您将要使用网络来连接到数据库。

##### 图 5\. 将网络设置更改为桥接

![将网络设置更改为桥接](img/a50908f5edec9f4ef76f9d508f73a47d.png)
...
您将需要使用一个数据库。如果您还没有数据库，那么您可以参考相应的教程下载。
...
虚拟机将使用 Sqoop 连接到数据库，这需要一个 JDBC 驱动程序。在虚拟映像中，您将需要一个面向您的数据库的 JDBC 驱动程序。您可以自行参照相应的教程学习安装 。

此处提供了 DB2 驱动程序： [`www.ibm.com/services/forms/preLogin.do?source=swg-idsdjs`](http://www.ibm.com/services/forms/preLogin.do?source=swg-idsdjs) 或 [`www-01.ibm.com/support/docview.wss?rs=4020&uid=swg21385217`](http://www-01.ibm.com/support/docview.wss?rs=4020&uid=swg21385217) 。
...
![运行 Hive 练习后的 Hadoop 作业指标的一个样例 Web 页面](img/1460c36ded81d7c4a2bb4e4141a972a7.png)
...
![本图显示了使用浏览器查看 HDFS](img/d082a4380c9433cbfaa57729a135e0df.png)

Cloudera 网站免费提供了一个更复杂的控制台。这个控制台提供了大量超出标准 Hadoop Web 界面的功能。请注意，图 8 所示的 HDFS 健康状态为 **Bad** 。

##### 图 8\. 由 Cloudera Manager 管理的 Hadoop 服务

![本图显示了由 Cloudera Manager 管理的 Hadoop 服务](img/dd923835e22bc7375c1b91905d4a78ca.png)
...
![Hue Beeswax GUI for Hive](img/c41e86bb086a78cb6278c71bc51b186f.png)

##### 图 10\. Hue Beeswax GUI for Hive，查看 Hiveql 查询

![Hue Beeswax GUI for Hive](img/2d6fbc909e74cd75c8ccf267690f1059.png)

##### 图 11\. Hue Beeswax 图形化浏览器，查看 Informix-DB2 联接结果

![Hue Beeswax 图形化浏览器](img/48db609fddf2ff77c6c0604169f5f5a9.png)
...
本文翻译自：Hadoop example: Hello World with Java, Pig, Hive, Flume, Fuse, Oozie, and Sqoop with Informix, DB2, and MySQL（2012-09-27）